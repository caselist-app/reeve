...
## The three real panel systems, and only three

1. **Global side panel**: `components/layout/app-content.tsx` (mechanism, owns the card token and 480px desktop width) + `components/layout/panel-shell.tsx` (chrome: title, optional description, X button, scrollable body) + `stores/side-panel-store.ts`. Use `PanelShell` for every global panel's content. Transient, opened/closed by user action from anywhere in the app.
...
Do not invent a fourth. `components/nav/tour-settings-panel.tsx` is a known bespoke exception (nav-rail slide-over, not a content panel) — do not use it as precedent for a new panel type.
...
`PanelShell` and `EditPanel` are chrome only: a header and a scrollable body, both rooted on a plain `flex h-full flex-col`. Neither carries the token, and neither should. `PanelShell` looks like a card because `app-content.tsx` wraps its content in one. `EditPanel` does not, because the schedule right column renders inside `<main>`, which is already a card.
...
1. Pick the right panel system (global, schedule right-column, or secondary) per the table above, not a new one.
