...
// The same class again, on the table that replaces day_sheets. Brief 42, REE-18.
//
// Worth stating why this is not just the block above with new column names. A
// day sheet was one wide row, so a partial save was a subset of twenty columns.
// An item is a narrow row, so the equivalent mistake is a panel that edits a
// location nulling the time, or a form that edits a time nulling the notes. The
// surface is smaller and the consequence is the same: a TM edits one thing and
// loses another, silently, and finds out on a day sheet that has already gone
// out over WhatsApp.
//
