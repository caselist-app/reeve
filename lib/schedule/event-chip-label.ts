 ...
 * synthetic (see the adapter's SYNTHETIC_END_MS), a "start–end" range
 * otherwise. Reads live event.start/end so the label tracks a drag or resize
 * gesture in progress, except for a continuesBefore block, which keeps its
 * real start.
 */
export function eventChipTimeLabel(event: CalendarEvent, timezone: string): string {
  ...
  if (event.syntheticEnd) return startLabel
  const endLabel = localTimeInZone(fromGridInstant(event.end.toISOString(), timezone), timezone)
