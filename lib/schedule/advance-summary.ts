...
// failed read must never render as five confident, plausible, wrong statuses.
export function summarise(row: SummaryRow | null, error: { message: string } | null): AdvanceSummary {
  ...
  return {
    error: null,
    statuses: {
      audio: (row.status_audio as AdvanceStatus) ?? 'not_started',
      lighting: (row.status_lighting as AdvanceStatus) ?? 'not_started',
      staging: (row.status_staging as AdvanceStatus) ?? 'not_started',
      hospitality: (row.status_hospitality as AdvanceStatus) ?? 'not_started',
      travel: (row.status_travel as AdvanceStatus) ?? 'not_started',
    },
  }
}

// The block's read. Lean by design: it selects only the five status columns
...
export async function fetchAdvanceSummary(
  ...
  const { data, error } = await supabase
    .from('show_advance')
    .select('status_audio, status_lighting, status_staging, status_hospitality, status_travel')
    .eq('show_id', showId)
    .maybeSingle()
