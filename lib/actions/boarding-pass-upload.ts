...
export async function uploadBoardingPassAction(
  ...
  const storagePath = `${tourId}/boarding-passes/${assignmentId}.pdf`
  ...
  // If so, update the existing row. Otherwise insert a new one.
  ...
  await scheduleBoardingPassSend(assignmentId)
