...
**Day calendar**: a react-big-calendar day-view grid (Brief 43, REE-55 onward), not the chronological list it replaced. `components/schedule/day-calendar.tsx`, a **client** component, because the grid, the drag gestures and the optimistic positions all need the boundary. It does not fetch: `components/schedule/day-content.tsx` (a Server Component) fetches the day's `DayRecords` and passes them in, and the calendar maps them to grid events through `lib/schedule/calendar-adapter.ts`. The three sources are unchanged: `day_items` (everything with a time that is not travel or a bed), `transport_segments`, and `hotel_stays` (check-in/check-out). A block's vertical position is its time; overlaps sit side by side (`dayLayoutAlgorithm="no-overlap"`), not stacked. Clicking a block opens its detail in the global side panel, read via `PanelShell`-based panels in `components/schedule/panels/`. Flights render a read-only `FlightCard` inside `transport-panel.tsx` (live status, tracked via AirLabs) with booking reference as the one editable field; every other mode is a plain edit form.
...
**Three things a block carries that a plain calendar does not.** An item with no time is not placeable, so it is listed in an "unpositioned" rail above the grid rather than dropped (a fresh show after Brief 42 has no times at all, so this is the normal state of a new day). A `transport_segment` whose departure falls after midnight is stored on the next day but shown in an "After midnight" tail under tonight's grid, because a 01:30 red-eye after a show is part of tonight from where the TM stands and RBC's day view has nowhere to draw it. And an item whose end is unknown (`ends_at` null) is drawn with a synthesised, display-only end and a dashed bottom edge; that end is never stored, and `fromDropOrResize` in the adapter strips it back to null on a move so a drag never invents a duration the TM did not set.
