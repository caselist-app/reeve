...
2. **Secondary panel** (`app/(app)/@secondaryPanel` Next.js parallel route slot + `components/layout/app-content.tsx`): a standalone, always-visible card to the left of main content, on routes that supply one. Not transient and not driven by a store: a route (e.g. `app/(app)/@secondaryPanel/tours/[id]/schedule/layout.tsx`) renders its content into the slot as a `layout.tsx` (not a `page.tsx`) so it persists across nested navigations exactly like any Next.js layout, the same way the schedule Dates list stays mounted across `?date=` clicks. `app/(app)/@secondaryPanel/default.tsx` renders nothing, so routes that don't use this are unaffected on a hard navigation. `AppContent` wraps whatever the slot renders in the card token at a fixed 230px width, `hidden lg:flex` (same breakpoint the schedule route already uses to swap to the mobile date strip). First and current use: the schedule Dates list (`components/schedule/date-sidebar.tsx`).
   ...
   **That persistence is also a trap, and it is the reason the Dates list goes stale.** Because the slot is a layout, `router.push()` and `router.refresh()` from a client component inside it do not re-resolve it. An action that adds or removes a day therefore has to call `revalidatePath` on the schedule route server-side; a client-side navigation afterwards will not pick the change up, and the new or deleted day sits in the sidebar until a hard reload. This bit `createRehearsal` and `deleteTourDate`, both of which push and never revalidate. If a change needs to show in the Dates list, the server action revalidates. There is no client-side route to it.
...
- **`null` means the TM cleared it. `undefined` means the form never sent it. Never collapse the two.** This is the single most destructive mistake made in this codebase, and it is invisible in review. A form that submits a subset of a record's fields, posted to an action that writes the whole row, will null everything it did not send, because Zod's `.optional()` yields `undefined` for an absent key and the row mapper then coerces that to `null` alongside genuinely cleared fields. `.default()` is worse: it invents a value for a key nobody submitted. Rules that follow, all three required:
  - **Never put `.default()` on a field in an action-facing schema** unless every caller submits it. `catering_type` had one, so the schedule day view's day-sheet panel (which sends 14 time fields and no catering) silently wrote `'none'` and nulled six catering columns every time a TM edited load-in.
  - **In the action, skip fields that are `undefined` and write fields that are `null`.** `updateDaySheet` in `lib/actions/shows.ts` is the reference loop.
  - **In a row mapper, omit a key that is `undefined` rather than mapping it to `null`.** `toRow` in `lib/actions/contacts.ts` is the reference: it coerced four `default_*` pay columns to null on every tour-context person edit, destroying rates the roster form had set.
  Both examples were live and destroying data until 2026-08-04. When you add a form that posts a subset of a record, assume this bug is present until you have checked the action.
