...
// Relative time reads in the item's own tour timezone, the same authority the
// schedule uses (localDateInZone, lib/schedule/datetime.ts), not the tour's
// timezone read through Intl at render time: that would resolve differently on
// the server (Node) and in the browser, flipping "Today" to "Yesterday" on
// first hydration for anyone not on UTC. timezone is nullable on tours, so a
// tour that never set one falls back to UTC, same as the schedule route does.
function relativeLabel(iso: string, timezone: string | null): string {
