...
- **Branch protection matches required checks by job name.** As of the Brief 41 merge, `End-to-end tests` was not in the rule on `main`, so that suite reports without blocking. Check with Matt before assuming a red suite stops a merge.
